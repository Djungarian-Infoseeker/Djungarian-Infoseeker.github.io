...
<body>
    ...
    <h3>2.1 网格定义</h3>
    <p>我选择将网格设置硬编码到模型（CESM1.2）中，虽然也有其他方法可以实现相同的效果。在此案例中，我修改了 <code>config_grid.xml</code> 文件，以包含新的案例配置：</p>

    <pre>
<code>&lt;GRID sname="1.9x2.5_1.9x2.5_AQUA" alias="f19_f19_AQUA" compset="(DOCN|XOCN|SOCN|AQUAP)"&gt;
    a%1.9x2.5_l%1.9x2.5_oi%1.9x2.5_r%r05_m%null_g%null_w%null
&lt;/GRID&gt;</code>
    </pre>

    <p>在这里，重要的部分是将海洋网格与大气网格设置为相同。此时，掩膜网格被设置为null。在 <code>config_grid.xml</code> 文件的后面部分，我还取消了域文件的设置，这些可以稍后通过命名列表进行设置。</p>
    ...
    <pre>
<code>create_newcase -case /glade/p/work/brianpm/my_cases/CAM5AquaSOM_noice -user_compset 2000_CAM5_SLND_SICE_DOCN%SOM_SROF_SGLC_SWAV -res f19_f19_AQUA -mach yellowstone</code>
    </pre>

    <p>到此为止，我们已经准备好设置案例：</p>

    <pre>
<code>cd /glade/p/work/brianpm/my_cases/CAM5AquaSOM_noice
./cesm_setup</code>
    </pre>
...
orb_obliq = 0.
orb_mvelp = 0.
orb_mode = ‘fixed_parameters’</code>
    </pre>
...
bnd_topo = ‘/glade/scratch/brianpm/AQUA_TOPO.nc’</code>
    </pre>
    ...
    <pre>
<code>env_build.xml
&lt;entry id="CAM_CONFIG_OPTS" value="-phys cam5-chem none" /&gt;</code>
    </pre>
    ...
    <pre>
<code>user_nl_docn
domainfile = ‘/glade/scratch/brianpm/domain.ocn.1.9x2.5_AQUAPLANET.nc’</code>
    </pre>

    <p>强迫文件在名为 <code>user_docn.streams.txt.som</code> 的文件中指定。该文件的内容可以从以前的平板海洋案例中复制。在这个示例中，文件内容如下：</p>

    <pre>
<code>user_docn.streams.txt.som
...
&lt;entry id="DOCN_SOM_FILENAME" value="/glade/scratch/brianpm/som.forcing.aquaplanet.Q0h50m.fv19.nc" /&gt;</code>
    </pre>
