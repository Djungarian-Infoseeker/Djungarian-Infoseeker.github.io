...
<p>
<b><i>热力学第一定律</i></b> 规定，物体的内能 <i>I</i> 可以因为对其做功、或者它对外做功，或者由于热量输入，或者由于化学成分的变化而发生改变。我们忽略最后一种效应，因此有：
</p>
...
<p>
其中 \( dW \) 是对物体所做的功，\( dQ \) 是输入到物体的热量，\( dI \) 是内能的变化。这些量都是按单位质量来考虑的。（例如，加热可以通过辐射、传导发生，功的作用发生在体积变化或压缩过程中。）右侧的量（用 \( d \) 表示）是“非完美”微分，\( Q \) 和 \( W \) 不能简单地看作“热量”和“功”的总和。内能的变化与热量和功密切相关，而不是能量的数量，而是能量的输入速率或流量。
</p>

<h4>热量输入 (Heat input)</h4>
<p>
虽然热量本身不是一个状态函数，但有一个与加热直接响应的状态函数，这就是熵。特别是在一个准静态或可逆过程的微小量中，当外部供应单位质量的微量热量 \( dQ \) 时，比熵 \( \eta \) 的变化可以表示为：
</p>
...
<h4>做功 (Work done)</h4>
<p>
在可逆过程中，对物体所做的功等于压力乘以体积的变化。如果功是正的，那么体积变化是负的。因此，当单位质量的物体受到微小功 \( dW \) 时，其热力学状态变化可以表示为：
</p>
...
<p>
其中 \( \alpha = 1 / \rho \) 是流体的比体积，\( p \) 是压力。
</p>

<p>
将方程 (1.43)-(1.45) 结合起来，我们得到：
</p>
...
<p>
这个表达式称为 <b>基本热力学关系</b> (fundamental thermodynamic relation)。接下来，我们将探讨它如何应用于流体。
</p>
...
<h3>1.6 位温与熵 (Potential Temperature and Entropy)</h3>
<p>
方程 (1.47) 适用于流体单元，而不是特定位置。内能和体积的变化率可以通过物质导数表示为：
</p>
...
<p>
其中 \( \dot{Q} \) 是热输入的速率。我们现在可以使用质量守恒方程 (1.27a) 来重写密度的时间导数，从而得到：
</p>
...
<p>
这是流体的 <b>热力学方程</b> (thermodynamic equation)，它告诉我们内能如何响应加热。
</p>
...
<p>
其中 \( c_v \) 是恒定体积下的热容。对于地球大气中的气体，\( c_v \) 本身是一个常数，因此方程 (1.49) 变为：
</p>

<p>
\[
c_p \frac{DT}{Dt} + \frac{p}{\rho} \nabla \cdot v = \dot{Q}.
\tag{1.51}
\]
</p>

<p>
方程 (1.51) 是大气科学中最常用的热力学方程之一。
</p>
...
<p>
从 (1.53a) 可以看出，\( c_p \) 仅仅是恒定压力下的热容。方程 (1.53b) 等价于 (1.51)，虽然在实际中不如 (1.51) 广泛使用。
</p>
