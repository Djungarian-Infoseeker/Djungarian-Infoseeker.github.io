...
<p>
由于地球和我们所知的大多数其他行星都非常接近球形，我们经常在球坐标系中表达方程。我们很少使用完整的球坐标形式，但我们的近似公式直接源于这些方程。读者可以跳过推导，直接查看主要结果，如(2.34)、(2.35)和(2.40)。
</p>
...
<ul>
    <li><strong>λ</strong> 是经度，即相对于极点的角度间隔。</li>
    <li><strong>θ</strong> 是纬度。</li>
    <li><strong>r</strong> 是从地球中心到该点的半径距离。</li>
</ul>

<p>
在某些其他坐标系中，我们也可以定义与地球相关的笛卡尔增量 \((x, y, z)\) 和 \((r \cos θ, r \sin θ, r)\)。对于标量变量 φ，其在球坐标系中的物质导数为：
</p>
...
<p>
其中速度分量对应于坐标 \((λ, θ, r)\) 分别为：
</p>

<p>
\[
(u, v, w) \equiv \left(\frac{1}{r \cos \theta} \frac{D \lambda}{Dt}, \frac{1}{r} \frac{D \theta}{Dt}, \frac{D r}{Dt}\right)
\]
</p>

<h4>2.2.2 标量的梯度和拉普拉斯</h4>
...
<p>
\[
\nabla^2 \varphi = \frac{1}{r^2 \cos \theta} \left[\frac{\partial}{\partial \lambda}\left(\cos \theta \frac{\partial \varphi}{\partial \lambda}\right)\right] + \frac{1}{r^2} \frac{\partial}{\partial \theta} \left(\cos \theta \frac{\partial \varphi}{\partial \theta}\right) + \frac{1}{r^2} \frac{\partial^2 \varphi}{\partial r^2}
\]
</p>

<h4>2.2.3 向量的散度和旋度</h4>

<p>
向量 <strong>B</strong> 的散度为：
</p>
<p>
\[
\nabla \cdot \mathbf{B} = \frac{1}{r^2 \cos \theta} \frac{\partial}{\partial \lambda}(B^\lambda r \cos \theta) + \frac{1}{r \cos \theta} \frac{\partial}{\partial \theta}(B^\theta \cos \theta) + \frac{1}{r^2} \frac{\partial}{\partial r}(r^2 B^r)
\]
</p>
...
\begin{vmatrix}
\mathbf{i} r \cos \theta & \mathbf{j} r & \mathbf{k} \\
\frac{\partial}{\partial \lambda} & \frac{\partial}{\partial \theta} & \frac{\partial}{\partial r} \\
...
<h4>2.2.4 单位向量的变化率</h4>

<p>
在球坐标中，单位向量定义如下：
</p>
<ul>
    <li><strong>î</strong>：指向东向，与纬线平行。</li>
    <li><strong>ĵ</strong>：指向北向，与子午线平行。</li>
    <li><strong>k̂</strong>：指向外向，与径向垂直。</li>
</ul>

<p>
这些向量的方向随位置而变化，这在几乎所有坐标系中都是如此，除了笛卡尔坐标系，其物质导数为零。
</p>
...
<p>
\[
\mathbf{\Omega} = \mathbf{\Omega}(j \cos \theta + k \sin \theta)
\]
</p>
...
<p>
\[
\mathbf{\Omega}_{flow} = -i \frac{v}{r} + j \frac{u}{r} + k \frac{u \tan \theta}{r}
\]
</p>
...
<p>
\[
\frac{D \mathbf{j}}{Dt} = \Omega_{flow} \times \mathbf{j} = -\frac{u}{r} \tan \theta \mathbf{i} - \frac{v}{r} \mathbf{k}
\]
</p>
<p>
\[
\frac{D \mathbf{k}}{Dt} = \Omega_{flow} \times \mathbf{k} = \frac{u}{r} \mathbf{i} + \frac{v}{r} \mathbf{j}
\]
</p>
