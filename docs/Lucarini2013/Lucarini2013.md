...
<body>
    <h1>3. 模型设置</h1>
    <p>
        通过多次运行数值模型并略微改变一个或多个参数的值来框定复杂系统的多稳定性特性，这确实是一个计算量很大的工作。
        如果我们研究像行星大气这样复杂的系统，并且处理一个二维参数空间，这个问题会更加棘手。在下文中，我们将关注一颗类地行星，
        并研究改变母恒星辐照度 \(S\) 和改变大气不透明度（由 CO\(_2\) 浓度调节）的影响。
        我们的目标是找到气候系统在 \((S, [\text{CO}_2])\) 空间中支持多稳定性的区域，并研究这些状态之间的转换。
        对于 \((S, [\text{CO}_2])\) 空间中找到多稳定性的每个点，我们将两个共存状态称为暖态 (W) 和雪球态 (SB)。
        在双稳态区域之外，气候状态将是 W 态或 SB 态。
    </p>
    <p>
        数值模拟是使用 Planet Simulator (PLASIM) 进行的。PLASIM（Fraedrich 等，2005）是一个运行速度快的中等复杂度气候模型，
        可以从 <a href="http://www.mi.uni-hamburg.de/plasim" target="_blank">这里</a> 免费获取。
        它的动力核心使用谱变换方法（Eliasen, Machenhauer & Rasmussen 1970；Orszag 1970）和半拉格朗日对流解算涡度、散度、
        温度、比湿和表面气压的对数形式的原始方程。大气与辐射之间的相互作用由简单但现实的长波（Sasamori 1968）和短波
        （Lacis & Hansen 1974）辐射方案处理。太阳辐射的处理包括季节性和日周期。次网格尺度上的未解析过程，如湿（Kuo 1965, 1974）
        和干对流、云形成和大规模降水（Stephens 1978；Stephens 等 1982；Slingo & Slingo 1991）、潜热和显热边界层通量、
        水平和垂直扩散（Louis 1979；Louis 等 1981；Laursen & Eliasen 1989）被参数化。
        该模型与一个 50 米深的薄海洋耦合，其中包含一个热力学海冰模型。所有模拟均在倾斜角（倾斜度）\(\delta = 23^\circ\) 下进行。
    </p>
    <h2>表 1. 参数和符号列表</h2>
    <table border="1">
        <tr>
        ...
        <tr>
            <td>\(\Omega_E\)</td>
            <td>地球的自转速率</td>
            <td>\(7.29 \times 10^{-5} \, \text{rad}^{-1}\)</td>
        </tr>
        <tr>
            <td>\(y_E\)</td>
            <td>轨道年</td>
            <td>\(360 \times 8.64 \times 10^4 \, \text{s}\)</td>
        </tr>
        <tr>
            <td>\(\delta\)</td>
            <td>倾斜角</td>
        ...
        <tr>
            <td>\(c_d\)</td>
            <td>干空气比热</td>
            <td>\(1004 \, \text{J} \, \text{kg}^{-1} \, \text{K}^{-1}\)</td>
        </tr>
        <tr>
            <td>\(c_{pw}\)</td>
            <td>混合层模型的比热</td>
            <td>\(4180 \, \text{J} \, \text{kg}^{-1} \, \text{K}^{-1}\)</td>
        </tr>
        <tr>
            <td>\(g\)</td>
            <td>重力加速度</td>
    ...
    <p>
        该模型以 T21 的谱分辨率运行（约 \(5.6^\circ \times 5.6^\circ\），并具有 10 个垂直层。虽然这一分辨率相对较粗，
        但预计对于缓慢旋转和潮汐锁定的行星来说，它足以合理地描述大气环流的大尺度特性。在更高的旋转速率（例如
        \(\Omega \geq 4 \Omega_E\)）下确实需要更高的水平分辨率，因为斜压结构的尺度（罗斯贝变形半径，参见例如
        Holton 2004）与 \(1 / \Omega\) 成比例缩放。
    </p>
    <p>
        最后，我们考虑母恒星辐照度 \(S\) 从 \(1165 \, \text{W} \, \text{m}^{-2}\) 到 \(1510 \, \text{W} \, \text{m}^{-2}\)，
        以及 CO\(_2\) 浓度 \([\text{CO}_2]\) 从 90 ppm 到 2880 ppm 的情况。
    </p>
</body>
</html>
