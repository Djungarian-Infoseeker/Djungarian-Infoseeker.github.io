...
<body>
    <h2>3. 示例应用：来自区域气溶胶排放的有效辐射强迫（ERF）</h2>
    <p>
        当在现今与前工业气溶胶排放之间切换时，CESM2展现出了强烈的ERF，这一点通过amip和rfmip实验可以看出，分别使用了2000年和前工业时期的SST。使用现实气溶胶排放的水球行星实验（aqua-pd与aqua-pi）显示出更强的ERF。在本节中，我们将应用水球行星配置，探讨ERF是否依赖于排放的空间模式。
    </p>
